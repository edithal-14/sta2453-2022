...
class ClusterWeightedModel(nn.Module):
    ...
    def Mstep(self):
        ...
        for g in range(self.G):
            term1 = torch.linalg.inv(self.X.T @ torch.diag(self.z[:,g]) @ self.X)
            self.betas[g,:] = term1 @ (self.X.T @ torch.diag(self.z[:,g]) @ self.y)
            self.xmus[g] = (self.x.T @ self.z[:,g]) / self.x.shape[0]
        for g in range(self.G):
            self.sigmas[g] = np.sqrt((
                self.z[:, g] * np.power((self.y - self.X @ self.betas[g, :].T), 2)
            ).sum() / self.z[:, g].sum())
        for g in range(self.G):
            self.xsigmas[g] = np.sqrt((
                self.z[:, g] * np.power((self.x - self.xmus[g]), 2)
            ).sum() / self.z[:, g].sum())
    ...
    def train_EM(self, max_iter=10):
        ...
        self.xsigmas.requires_grad = False
        self.xmus.requires_grad = False 
        for it in tqdm(range(max_iter), desc="Running EM algo"):
            self.z = self.Estep(self.X, self.y)
            self.Mstep()

            if it % 1 == 0:
                tqdm.write(f"Loss: {self.objective_fn(self.X, self.y)}")
